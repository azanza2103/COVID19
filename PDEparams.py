...
class PDEmodel:
    ...
    def plot_profiles(self):
        ...
        for i, pname in enumerate(self.param_names):
            ...
            plt.semilogy(data.value.values, data.error.values, c=colours[5])
            #plt.plot(data.value.values, data.error.values, c=colours[5])
            print('Min for '+pname+' parameter: ',np.min(data.error.values))
            if np.max(data.error.values) > 250*np.min(data.error.values):
                ...
                plt.ylim(0, 250*abs(np.min(data.error.values)))
            else:
